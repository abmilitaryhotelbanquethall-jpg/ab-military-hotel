...
const initialStoreItems = [
    ...
    // Add more items as needed (144 total)
];
...
async function initializeStoreItems() {
    try {
        ...
        if (!snapshot.exists()) {
            ...
            const today = '2025-12-01'; // FIXED DATE as per requirement
            const itemsData = {};
...
function setupRealtimeListeners() {
    ...
    dashboardRef.on('value', (snapshot) => {
        loadDashboard();
        updateMeatFridge(); // ENHANCEMENT #4
        updateStockStatus();
    });
...
async function loadDashboard() {
    try {
        ...
        document.getElementById('todayExpenses').textContent = '₹' + totalExpense.toFixed(2);
        document.getElementById('todayPurchases').textContent = '₹' + totalPurchase.toFixed(2);
        document.getElementById('storeValue').textContent = '₹' + storeValue.toFixed(2);
...
// ==========================================
// MEAT FRIDGE INVENTORY (ENHANCEMENT #4)
// ==========================================
            ...
            grid.innerHTML = Object.entries(meatCategories).map(([name, data]) => `
                <div class="meat-card">
                    ...
                    <div class="meat-stat">
                        ...
                        <span class="value">${typeof data === 'number' ? '0' : data.stock} ${typeof data === 'number' ? 'kg' : data.unit}</span>
                    </div>
                    <div class="meat-stat">
                        <span class="label">Fresh Today:</span>
                        <span class="value">${typeof data === 'number' ? '0' : data.purchased} ${typeof data === 'number' ? 'kg' : data.unit}</span>
                    </div>
                    <div class="meat-stat">
                        <span class="label">Total Available:</span>
                        <span class="value">${typeof data === 'number' ? '0' : data.total} ${typeof data === 'number' ? 'kg' : data.unit}</span>
                    </div>
                </div>
            `).join('');
        }
...
function calcRate() {
    ...
    if (qty > 0) {
        document.getElementById('purchaseRate').value = (amount / qty).toFixed(4);
    }
}
    ...
    try {
        ...
        tbody.innerHTML = Object.entries(purchases).map(([key, item]) => `
            <tr>
                ...
                <td>${(item.remaining || 0).toFixed(2)} ${item.unit}</td>
                <td>
...
async function loadDayItems() {
    try {
        ...
        tbody.innerHTML = Object.entries(purchases).map(([key, item]) => {
            const totalAvailable = parseFloat(item.totalAvailable) || 0;
            const remaining = parseFloat(item.remaining) || 0;
            const percentage = totalAvailable > 0 ? Math.round((remaining / totalAvailable) * 100) : 0;
            ...
            return `
                <tr>
                    <td>${item.itemName}</td>
                    <td>${item.category}</td>
                    <td>${(item.stockInStore || 0).toFixed(2)}</td>
                    <td>${(item.purchasedQty || 0).toFixed(2)}</td>
                    <td>${(item.totalAvailable || 0).toFixed(2)}</td>
                    <td>${(item.ratePerUnit || 0).toFixed(2)}</td>
                    <td>${(item.usedQty || 0).toFixed(2)}</td>
                    <td>${(item.remaining || 0).toFixed(2)}</td>
    ...
    try {
        ...
        tbody.innerHTML = allPurchases.map(p => `
            <tr>
                ...
                <td>${p.category}</td>
                <td>${p.date}</td>
                <td>${(p.purchasedQty || 0).toFixed(2)}</td>
                <td>${p.unit}</td>
                <td>₹${(p.ratePerUnit || 0).toFixed(2)}</td>
